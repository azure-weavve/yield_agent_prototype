...
root_lot A45Z5 (물리 묶음, 거의 항상 25매)
 ...
 │    ├── wafer_id A45Z5_01   ← {root_lot_id}_{wafer_no}. lot_id 기준이 아님!
 │    └── ... A45Z5_20
 └── lot_id A45Z5.U2  (평가랏 — ".1" 외 전부)
      └── A45Z5_21 ... A45Z5_25
```

- **wafer_id 는 root_lot 기준**이라 wafer_id 만으로 양산/평가 소속을 알 수 없다.
...
- 현재 코드는 ID 를 파싱하지 않음(전부 파라미터 바인딩 완전 일치) →
  ID 형식 자체는 코드 수정 없이 수용됨. 확인: 2026-07-14 전수 검색.
...
| 컬럼 | 규칙 |
|------|------|
| `wafer_id` | 원천 그대로 `{root_lot_id}_{wafer_no}` — 전 시스템 공통 조인 키 |
