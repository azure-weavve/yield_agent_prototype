...
> **비정규 스텝이면 뒤에 `EC` 가 붙는다** = `"AA110000EC"` (2026-08-03 확인). 정상적으로
> 실려 오는 값이므로 접미를 **떼지 않고 그대로** 싣는다 — 떼면 정규 스텝과 같은 키로
> 뭉쳐 commonality 의 분모가 조용히 달라진다. 붙인 채로 두면 별개 스텝 키가 된다.
> (사내에 문자3+숫자5 체계도 있으나 이 팀 데이터에는 안 나온다. `validate()` 검사 2-1 의
> 정규식이 2+6(+`EC`)로 좁혀져 있으니, 그 체계를 받게 되면 거기부터 넓힐 것.)
