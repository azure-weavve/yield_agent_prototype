...
- 위치: `ya_config.py` `EDS_HTTP_VERIFY`
- 문제: http 모드를 켜는 것만으로 인증서 검증 없이 사내 데이터 전송
- 처방: 사내 루트 인증서(.pem) 확보 → `EDS_HTTP_VERIFY = "인증서경로"` 를 기본값으로, 우회는 개발 환경에서만 명시적으로
- **2026-08-04 부분 진행**: `EDS_MODE` 와 함께 env 로 전환했다 — `.env` 에 `EDS_HTTP_VERIFY=/경로/.pem`
  을 주면 검증이 켜진다(그 전에는 코드를 고쳐야 했다). **기본값은 여전히 `False`(우회)이고,
  그것을 뒤집는 것이 이 항목의 본체다.** 뒤집으면 .pem 을 아직 못 구한 환경에서 EDS 가
  통째로 막히므로, 사내 인증서 확보를 확인한 뒤에 한다.
