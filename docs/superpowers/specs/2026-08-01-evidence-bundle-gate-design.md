...
> 4. **혼합 상태(한 가설은 `no_signal`, 다른 가설은 `status="ok"` 이나 통과 후보 없음)는
>    §12 가 다루지 않았다.** 병합 전 최종 검토(2026-08-02)에서 지적됨 — §12 에 한 문장
>    추가만 했다(사용자 결정: 지금 판정 코드는 바꾸지 않는다). 상세는 §12 참조.
...
**혼합 상태(한 가설은 `no_signal`, 다른 가설은 `status="ok"` 이나 통과 후보가 판별선을
못 넘음)도 지금 판정은 2번 규칙(`"no_signal" in bundle.statuses.values()`)만 보므로
`no_signal` 로 끝난다** — 실제로는 다른 가설이 약한 후보를 냈을 수 있는데도 "lot 밖
대조군이 필요하다"로 보고된다. (최소 수정안: 2번 규칙의 `not bundle.passing()` 을
`not bundle.claims` 로 바꾸면 후보를 하나도 못 낸 경우에만 `no_signal` 이 된다.) 더미
데이터에는 이 혼합 상태가 나오지 않아 어느 판정이 옳은지 실데이터로 확인한 뒤 정한다 —
지금은 판정 코드를 바꾸지 않는다.
