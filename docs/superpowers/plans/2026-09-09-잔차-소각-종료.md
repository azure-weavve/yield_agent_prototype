...
**Architecture:** 합집합 결정을 헬퍼 하나(`_evidence_groups`)로 모아 세 자리가 같은 규칙을 쓰게 한다. `_fold_key`(C 브랜치 확정 계약)는 건드리지 않는다 - "지목 가능한 통과 후보가 있으면 잔차를 안 싣는다"(B §10)를 세 자리 모두에서 지키면 통과 claim 과 잔차가 한 묶음에 섞이는 일 자체가 안 일어난다. M4 는 `(2a)` 바로 뒤 `(2)` 앞에 새 분기로 들어가고, 판정 상태 판별은 `_no_separation_state` 하나에만 적는다(게이트와 물러섬 안내가 같은 것을 본다).
...
세 자리가 같은 결정을 하는데 (2a) 에만 적혀 있었다. (4) 와 백스톱이
같은 규칙을 쓰기 전에 결정 자리를 하나로 만든다 - 규칙을 세 번 적으면
...
- [ ] 전축 ok · 통과 0 · 잔차 0 -> `no_separation`, 최고 점수가 판정문에 찍힌다.
