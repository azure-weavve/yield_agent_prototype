...
> **각주(2026-09-12, m1):** 위 절의 "`p == p_min_possible` 은 '귀무가 관측을 한 번도
> 못 넘었다' 는 뜻이다" 에는 예외가 하나 생겼다 — **참조 회차가 0이면 p 도 바닥도
> 1.0 이라 둘이 같지만 뜻은 "잴 것이 없었다" 다**(참조집합을 표본 크기가 같은 회차로
> 좁힌 2026-08-29 변경). 소비자는 두 숫자를 비교하지 말고 도구가 세어 싣는
> `p_at_floor` 를 읽는다(`tools/commonality.py::_null_distribution`).
...
> **각주(2026-09-12, m6):** 위 문구 중 **"표본이 작아" 는 지금 코드에 없다.**
> 바닥을 정하는 것은 참조 회차이고, 그것은 섞을 배치가 적어도(작은 표본) 줄고
> 순열 회차 예산이 작아도 줄어든다 - 실측: 10대10(경우의 수 184,756)에 회차 5면
> 바닥이 0.1667 이다. 현재 문구는 "참조 회차가 적어" 로 시작하고 두 원인을 가르는
> 법(`n_reference` 와 `n_permutations_total` 비교)까지 적는다.
...
> **각주(2026-09-12, m1·m6):** 위 블록과 그 앞 문장의 "문구를 바꾸지 마라" 는 더
> 이상 맞지 않다.
> yaml 네 블록의 이 문단은 두 번 고쳐졌다 - (1) "표본이 작아" -> "참조 회차가 적어"
> (바닥의 원인은 표본이 아니라 참조 회차다), (2) `p_at_floor` 를 읽으라는 문장 추가
> (반올림된 p 와 바닥을 LLM 이 직접 비교하지 않도록). 현재 문구는
> `domain/hypotheses.yaml` 이 원본이고, `tests/test_registry.py` 가 두 규칙을 잠근다.
...
**1000회가 60초를 넘으면 멈추고 보고한다.** (2026-08-10 사용자 확정: 한 번 돌리고 기다릴 수 있는 분석이라 60초까지는 허용한다. 넘으면 기본 반복 횟수를 낮출지 최적화를 더 할지 사람이 정한다.)
