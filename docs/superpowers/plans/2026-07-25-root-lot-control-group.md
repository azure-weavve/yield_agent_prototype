...
Run: `PYTHONUTF8=1 python main.py R2418_01 R2418_02 R2418_03 R2418_04`
확인할 것: 분할 lot 케이스가 `control_insufficient` 로 끝나지 않고 `ETCH5_B` 결론까지 가는지.

**정정 (실행 중 발견):** 원안은 `main.py R2418_01` 단독 입력이었는데, 분할 lot 타깃에는
클러스터 임베딩을 심지 않아 EDS 형제 묶기가 안 되고 `isolated` 로 끝난다. 대조군 규칙을
보려면 EDS 묶기를 건너뛰는 **그룹 입력**을 써야 한다. 임베딩을 심는 대신 그룹 입력으로
검증하는 쪽을 택했다 — Stage 2 의 계약은 대조군 선정이지 형제 묶기가 아니다.

**실행 결과 (2026-07-25):**
- `python main.py` → 결론 `ETCH9_B`(1.0) **유지**. 대조군 67장 → **78장**으로 늘고
  요약에 `수율 중앙값 95.3, 임계 90.0 미만 10장` 이 붙는다.
- 그룹 입력 → 결론 `Etch 공정 ETCH5_B 편중(분리 점수 1.0, 불량군 4장 전용)`.
  옛 lot 기준이면 `R2418.1` 에 비타깃이 0장이라 `control_insufficient` 였을 케이스다.
