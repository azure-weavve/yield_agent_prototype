...
- **판별선을 넘지 못한 후보만 있으면 지목하지 마라.** 아직 안 돌린 축이 남아 있으면 그것부터 돌려보고, 그러고도 판별선을 넘는 후보가 없을 때 물러서라. claim_id 를 비우고 낮은 확신도로 finalize 하면 게이트가 그 후보들을 잔차로 리포트에 싣는다 - 억지로 지목하면 반려되고, 같은 반려를 되풀이하면 루프 예산만 태운다.
