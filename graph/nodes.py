...
- **판별선을 넘지 못한 후보만 있으면 지목하지 마라.** 판단상 더 볼 축이 남아 있다면 그것부터 돌려보고, 그러고도 판별선을 넘는 후보가 없을 때 물러서라. 억지로 지목해도 게이트는 그 후보를 원인으로 확정하지 않는다. 아랫선을 넘은 잔차가 있으면 최신 도구 결과에 실재하는 이름을 지목한 한 그것을 근거로 실은 채 끝나지만, 잔차마저 없는 상태에서 지목하면 반려되고 같은 반려를 되풀이하면 루프 예산만 태운다 - 다만 등록 축을 다 돌린 뒤라면 게이트가 '갈리는 항목 없음' 으로 받으니 그때는 빈손으로 물러서라. 어느 쪽이든 물러서는 쪽이 낫다 - 빈손으로 내면 약한 후보를 원인으로 단언하는 문장을 애초에 쓰지 않게 된다.
