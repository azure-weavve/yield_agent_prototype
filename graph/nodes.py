...
- **2단 센서(compare_sensor_distribution) 후보의 claim_id 는 근거 인용용이다.** finalize 로 지목할 수 있는 것은 가설 도구(hyp_*)가 발급한 claim_id 뿐이다 - 센서는 스텝당 수백 개라 다중비교 보정 없이 효과크기 순위만으로는 우연한 분리를 가릴 수 없어 반려된다. 그렇다고 안 돌려도 된다는 뜻은 아니다: '왜' 를 채우는 근거이고, 그중 효과크기가 판별선을 넘은(passes=true) 후보만 게이트가 리포트에 함께 싣는다.
- **판별선을 넘지 못한 후보만 있으면 지목하지 마라.** 판단상 더 볼 축이 남아 있다면 그것부터 돌려보고, 그러고도 판별선을 넘는 후보가 없을 때 물러서라. claim_id 를 비우고 낮은 확신도로 finalize 하면 게이트가 그 후보들을 잔차로 리포트에 싣는다 - 억지로 지목하면 반려되고, 같은 반려를 되풀이하면 루프 예산만 태운다.
