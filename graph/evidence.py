...
class Claim:
    ...
    # 오므로 여기서 두 숫자를 == 로 비교하면 참조 회차가 13,333~19,999 이거나 40,000 이상일 때 귀무가
...
def format_evidence_line(claim: dict) -> str:
    ...
    if p is not None:
        ...
        # 4자리로 반올림돼 있어 참조 회차가 13,333~19,999 이거나 40,000 이상일 때
        # 같은 숫자가 된다(앞 구간은 1/13334 과 2/13334 이 둘 다 0.0001, 뒤 구간은
        # 둘 다 0.0). 그러면 귀무가 넘은 후보에 정반대 딱지가 붙는다.
