...
3. **스텝마다 계측 wafer 가 달라지면 살아난다** (위 표 3행). 조합 하나가 보는
   wafer 수는 그대로지만 `seen` 의 합집합이 커져 순열 공간이 회복된다.

**사내는 스텝마다 같을 수도 다를 수도 있다** (2026-08-12 확인. 예: A 스텝에서 3·4·5번을
재고 다음 계측 스텝에서 12·13·14번을 잰다). 즉 **유리한 쪽이 섞여 있다.** 다만 파워가
lot 마다 달라진다 — 어떤 lot 이 전 스텝을 같은 슬롯으로만 재면 그 lot 의 기여는 위
...
가는 쪽이다. `graph/evidence.py` 의 `dominates` 가 순열 p 를 **축을 가로지르는 유일한
