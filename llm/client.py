...
class LLMClient(ABC):
    ...
    def generate_report(
        ...
        `rolled_up_as` 는 **같은 설명의 굵은 해상도**(설비 ⊃ 챔버)라 경합하는 근거가
        아니다 - 구분이 안 된다고 쓰면 당연한 소리가 되고 진짜 미해결과 섞인다.
        대조군에 그 굵은 이름을 지났으면서 세밀한 이름은 아닌 wafer 가 없다는
        사실(대조군 범위)로 적어라.
...
class OpenAILLMClient(LLMClient):
    ...
    def generate_report(self, target_wafers, target_source, target_group, status_summary,
        ...
        sys = (
            ...
            "rolled_up_as 는 다르다 - 같은 설명을 굵은 해상도로 부른 이름(설비 PHOT7 은 "
            "챔버 PHOT7_B 를 포함한다)이지 경합하는 다른 근거가 아니다. 둘 중 무엇인지 "
            "구분되지 않는다고 쓰지 마라. 대신 대조군에 그 굵은 이름을 지났으면서 세밀한 "
            "이름은 아닌 wafer 가 없다는 사실(대조군 범위의 한계)을 적고, 후속 조치는 "
            "다른 축을 더 보는 것이 아니라 대조군 범위를 넓히는 것이다."
        )
        user = (
        ...
        if claims:
            user += (f"\n게이트가 확인한 근거 {len(claims)}건 "
                     ...
                     f"설명을 굵은 해상도로 부른 것뿐이니 경합하는 근거로 쓰지 말고 "
                     f"대조군 범위의 한계로 적어라): "
