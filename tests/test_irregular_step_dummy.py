...
def test_irregular_step_is_caught_only_by_the_passage_axis():
    ...
    타깃 4장이 비정규 스텝을 **제각각 다른 설비·PPID 로** 거치므로, 그 축들에서는
    후보가 1/4 씩 쪼개져 판별선(score 0.5 / 타깃 2장)을 못 넘는다. 통과 여부로 보면
    타깃 4/4 · 대조군 0/4 로 완전히 갈린다.
    """
    for hid in ("eqp_ch_commonality", "ppid_commonality"):
        res = _run(hid)
        assert res["status"] == "ok"                       # 계산은 됐다
        assert res["candidates"]                           # 후보도 냈다
        assert not [c for c in res["candidates"] if c["passes"]], f"{hid} 가 잡으면 안 된다"
