...
def _state_with_unencodable_content():
    ...
    2026-08-04 스윕으로 **이 저장소의 출력 리터럴에는 em-dash 가 하나도 없다.**
    그래도 이 테스트는 살아 있어야 한다 - 방어가 필요한 이유가 저장소 문구가 아니라
    운영 LLM 산출물·wafer_id·DB 경로처럼 이 저장소가 통제하지 못하는 값이기 때문이다.
    그래서 리터럴을 찌르지 않고 상태값에 직접 넣는다.
    """
    return {
