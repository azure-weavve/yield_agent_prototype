...
> 더미에서는 여전히 동작하지만 `LEGACY_TOOLS_ENABLED` 기본값이 `0`이라 LLM 도구 목록에서
> 기본적으로 빠집니다 — `=1` 로 켜면 노출되지만, 실데이터에서는 죽은 도구가 루프 예산을
> 태우거나 `find_counterexamples`처럼 라벨 없음을 "반례 없음(특이성 확인)"으로 오독할 수 있어
> 기본은 꺼둔 채입니다.
...
적혀 있지 않고(라벨 컬럼은 전 행 NULL), 수율과 아래 원인 신호로만 드러납니다. 원인 신호는 두 층에 있습니다 —
