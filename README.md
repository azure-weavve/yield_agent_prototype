...
| `no_comparable_data` | 등록 가설을 전부 돌렸는데 계산 자체를 못 했다 (`no_paired_stratum`·`insufficient_group`, 도구 실패로 못 돈 축이 없을 때) | **근거를 못 찾은 게 아니라 볼 것이 없었습니다** — 적재 범위와 추출 조건을 확인합니다. legend 와 무관한 그룹 수준 사실이라 남은 가설을 기다리지 않고 첫 판정에서 끝냅니다 |
| `tool_failure` | 남은 축이 없는데 못 본 이유가 가설 도구의 **실행 실패**다 (계산된 축이 하나도 없다) | **볼 것이 없었던 게 아니라 조회가 실패했습니다** — DB·서비스 상태를 확인하고 재실행합니다. `no_comparable_data` 와 사실은 겹칠 수 있지만 조치가 다릅니다(적재 범위가 아니라 인프라). 어느 축이 터졌는지는 리포트의 `[커버리지]` 줄에 나갑니다 |
